...
After finding a valid triplet we need to move both pointers inward because keeping both pointers could cause reusing of the same values and cause duplicates or inf loops. Since the sum is already correct, moving both would basically make sure that we are searching for new pairs.
